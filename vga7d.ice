    ...
    "graph": {
      "blocks": [
        ...
        },
        {
          ...
          "type": "basic.code",
          "data": {
            "code": "// Parellel interface\nreg [7:0] d_latch = 0, c_latch = 8'h70;\nreg [2:0] rs_latch = 0;\nreg [11:0] waddr = 0;\nreg [15:0] d_out = 0;\nreg [3:0] req_filter = 0;\nreg request = 0;\nreg we = 0;\n\nwire req = !cs && !wr;\n\nalways @ (posedge clk)\nbegin\n    req_filter <= {req_filter[2:0], req};\n    request <= (req_filter == 4'b0011);\n    rs_latch <= rs;\n    d_latch <= d;\n    d_out <= {c_latch, d_latch};\nend\n\nwire [11:0] next_addr = ((waddr == 2399) || (rs_latch == 3'b100))?0:waddr + 1;\nwire write = request & (rs_latch == 3'b000);\nwire set_waddr = request & (rs_latch[1:0] == 2'b00);\nwire set_color = request & (rs_latch == 3'b001);\n\nalways @ (posedge clk)\nbegin\n    we <= write;\n    if (set_waddr) waddr <= next_addr;\n    if (set_color) c_latch <= d_latch;\nend\n",
            "params": [],
            "ports": {
              "in": [
                {
